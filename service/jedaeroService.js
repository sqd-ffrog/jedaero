...
const getNonMeetingLecturesModelList = async ({year, semester}) => {
    console.debug(`year: ${year}, semester: ${semester}`);
    const {username: account, password: baseInfo} = await Keychain.getGenericPassword();
    const { password } = JSON.parse(baseInfo);
    let res;
    try {
        res = await Dreamy.getNonMeetingLecturesModelList({year, semester});
    } catch (err) {
        console.error(err);
        await Dreamy._openSession(account, password);
        res = await Dreamy.getNonMeetingLecturesModelList({year, semester});
...
const getNonMeetingLectureModelDetail = async({year, semester, classCode}) => {
    console.debug('year: ', year, 'semester : ', semester, 'classCode : ', classCode);
    const {username: account, password: baseInfo} = await Keychain.getGenericPassword();
    const { password } = JSON.parse(baseInfo);
    let res;
    try {
        res = await Dreamy.getNonMeetingLecturesModelDetailList({year, semester, classCode});
    } catch (err) {
        await Dreamy._openSession(account, password);
        res = await Dreamy.getNonMeetingLecturesModelDetailList({year, semester, classCode});
    } finally {
